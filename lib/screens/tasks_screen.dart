...
class _TasksScreenState extends State<TasksScreen> {
  Future retrieveTasks() async {
    List<Task> savedTasks = await TodoProvider().getTasks();
    Provider.of<TaskList>(context).addSavedTasks(savedTasks);
    int activeTasksSaved =
        (savedTasks.where((task) => (task.isDone == false)).toList()).length;
    Provider.of<TaskList>(context).countActiveTasks(activeTasksSaved);
  }
                  ...
                  Text(
                    (taskData.activeTasks == 0)
                        ...
                        : (taskData.activeTasks == 1)
                            ? '1 Active Task'
                            : '${taskData.activeTasks} Active Tasks',
                    style: TextStyle(
                        color: Colors.white,
                        fontSize: 18.0,
